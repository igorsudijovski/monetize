...
export default function Dashboard({app, restriction} : {app: ApplicationsEntity | null, restriction: Restriction | null}) {
  ...
  const refreshSubscriptions = () => {
    if (app != null) {
      axios.get<{"subscriptions": ApplicationSubscriptionsEntity[]}>("/api/dashboard/:appId/subscriptions".replace(":appId", app.id.toString()), { withCredentials: true })
          .then(response => {
  ...
  useEffect(() => {
    if (app != null) {
      axios.get<{"revenues": RevenuePerApp[]}>("/api/dashboard/:appId/revenue".replace(":appId", app.id.toString()), { withCredentials: true })
          .then(response => setRevenues(response.data.revenues))
          .catch(error => console.error('Error fetching revenue data:', error));
      axios.get<{"generatedKeys": number}>("/api/dashboard/:appId/generated-keys".replace(":appId", app.id.toString()), { withCredentials: true })
  ...
  const onSwap = (subId: string, direction: "left" | "right") => {
    axios.put(`api/${app?.id}/app-subscriptions/${subId}/${direction}`, {}, { withCredentials: true })
        .then(response => {
    ...
    <Box sx={{ p: 4 }}>
      ...
      {app != null ? (
          ...
          <Box sx={{ mt: 3 }}>
            ...
            {restriction?.showGraph === true && revenues.length > 0 && (
                <Box>
                  <Divider sx={{ my: 4 }} />
            ...
            <Box sx={{ mt: 3 }}>
              <Card sx={{ p: 2, display: "flex", justifyContent: "space-between", alignItems: "left" }}>
                ...
                <SubscriptionDragGrid subscriptions={subscriptions} onEdit={onEdit} onActivate={onActivate} onDeactivate={onDeactivate} onSwap={onSwap}
                                      onBuy={() => setSnackbar({ open: true, severity: "success", message: "When pressing this button the user buy the subscription type" })} />
              ...
              >
                {subscriptions.filter(s => s.active).map((sub) => (
                    <Grid key={"grid" + sub.id} item xs={6} md={4} display="flex">
                      <SubscriptionCard {...mapToSubscriptionCardApp(sub)} onBuy={() => setSnackbar({ open: true, severity: "success", message: "When pressing this button the user buy the subscription type" })} />
                    </Grid>
                ))}
              </Grid>
            </Card>
